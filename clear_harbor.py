...
class ClearHarbor(object):
    ...
    def fetch_del_repos_name(self, pro_id):
        ...
        for repo in repos_res.json():
            #print(repo)
            tag_url = self.repos_url + "/" + repo + "/tags"
            # 项目镜像仓库的所有tags,按创建时间排序
            tags = self.client.session.get(tag_url).json()
            #print(len(tags))
            #if repo["tags_count"] > self.num:
            if len(tags) > self.num:
                #print("镜像仓库名称：",repo)
                self.del_repos_name.append(repo)
                #print("镜像名：%s 数量：%s" %(self.del_repos_name,len(tags)))
...
if __name__ == "__main__":
    clean = ClearHarbor(harbor_domain="172.16.16.253",
                        username="admin",
                        password="admin",
                        num=40)
    clean.work()